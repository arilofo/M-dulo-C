...
BEGIN
    ...
    -- [3.6] Guardar fecha anterior para auditoría RNF04
    SELECT fecha_programada INTO v_fecha_anterior
    FROM   CuentaPorPagar WHERE id_cxp = p_id_cxp AND activo = 1;

    -- [3.1] Validar que la cuenta existe y está activa
    IF v_fecha_anterior IS NULL THEN
        -- Verificar si la cuenta existe en absoluto (sin filtro activo)
        IF NOT EXISTS (SELECT 1 FROM CuentaPorPagar WHERE id_cxp = p_id_cxp) THEN
            SIGNAL SQLSTATE '45000'
                SET MESSAGE_TEXT = 'La cuenta por pagar no existe.';
        ELSE
            SIGNAL SQLSTATE '45000'
                SET MESSAGE_TEXT = 'La cuenta por pagar está inactiva o no tiene saldo pendiente.';
        END IF;
    END IF;

    -- [3.2] Impedir programar pago sobre cuentas ya pagadas
    IF NOT EXISTS (
        SELECT 1 FROM CuentaPorPagar
        WHERE  id_cxp          = p_id_cxp
          AND  activo          = 1
        ...
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'No se puede programar pago: la cuenta ya está pagada o no tiene saldo pendiente.';
    END IF;
    ...
    -- [3.6] Auditoría RNF04: cambio de fecha de pago programada
    INSERT INTO BitacoraAuditoria (tabla_afectada, operacion, id_registro, usuario, valor_anterior, valor_nuevo, fecha_evento)
